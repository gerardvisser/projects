/*
   ...
   Copyright (C) 2008 - 2014 Gerard Visser.
...
void* mem_allocate (const char* typeName, size_t sizeInBytes, int ptype) {
  ...
  dbgMemStats::debugBytes ()->add (sizeof (dbgRefInfo_t));
  dbgMemStats::allocatedBytes ()->add (sizeInBytes);

  void* result = malloc (sizeInBytes);
  dbgMemStats::list ()->add (new dbgRefInfo_t (result, typeName, sizeInBytes, ptype));
...
void mem_deregister (void* pointer) {
  ___BTPUSH;
  pthread_mutex_lock (&mutex);
  dbgRefInfo_t* refInfo = dbgMemStats::list ()->remove (pointer);
  if (refInfo == NULL) {
  ...
  dbgMemStats::debugBytes ()->sub (sizeof (dbgRefInfo_t));
  delete refInfo;
  pthread_mutex_unlock (&mutex);
  ___BTPOP;
}
...
void mem_register (const char* typeName, size_t sizeInBytes, int ptype, const void* pointer) {
  ...
  dbgMemStats::requestedBytes ()->add (sizeInBytes);
  dbgMemStats::debugBytes ()->add (sizeof (dbgRefInfo_t));
  dbgMemStats::list ()->add (new dbgRefInfo_t (pointer, typeName, sizeInBytes, ptype));
  pthread_mutex_unlock (&mutex);
  ___BTPOP;
}
