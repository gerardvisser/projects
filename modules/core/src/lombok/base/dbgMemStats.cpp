...
static dbgRefInfoHashList_t* _list = NULL;

static dbgAllocInfo_t* _debugBytes = NULL;
static dbgAllocInfo_t* _allocatedBytes = NULL;
static dbgAllocInfo_t* _requestedBytes = NULL;
...
dbgAllocInfo_t* dbgMemStats::allocatedBytes (void) {
  if (_allocatedBytes == NULL) {
    _allocatedBytes = new dbgAllocInfo_t ();
    debugBytes ()->add (sizeof (dbgAllocInfo_t));
  }
  return _allocatedBytes;
}

dbgAllocInfo_t* dbgMemStats::debugBytes (void) {
  if (_debugBytes == NULL) {
    _debugBytes = new dbgAllocInfo_t ();
    _debugBytes->add (sizeof (dbgAllocInfo_t));
  }
  return _debugBytes;
}

dbgRefInfoHashList_t* dbgMemStats::list (void) {
  if (_list == NULL) {
    _list = new dbgRefInfoHashList_t ();
    debugBytes ()->add (sizeof (dbgRefInfoHashList_t));
  }
  return _list;
}
...
dbgAllocInfo_t* dbgMemStats::requestedBytes (void) {
  if (_requestedBytes == NULL) {
    _requestedBytes = new dbgAllocInfo_t ();
    debugBytes ()->add (sizeof (dbgAllocInfo_t));
  }
